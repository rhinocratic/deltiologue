(ns rhinocratic.deltiologue.web.router
  (:require
   [reitit.ring :as ring]
   ...
   [rhinocratic.deltiologue.web.api.routes :as api-routes]))
...
(defn routes
  ...
  [["/api" (#'api-routes/routes db)]
   ...
   ["/swagger.json" {:get {:no-doc true
                           :swagger {:info {:title "deltiologue api"}}
...
(defn app
  ...
  (let [rtr (router profile db)
    ...
    (condp = profile
      :dev (ring/reloading-ring-handler handler-fn)
      :prod (handler-fn))))
